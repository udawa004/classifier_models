...
public class nn_regression {
		...
		public static Double[] modelTrain(double lambda, List<Integer> y, List<ArrayList<Double>> trainData){
			...
			//double[][] xw = new double[trainData.size()][trainData.get(0).size()];
			double[] xw = new double[y.size()];
			//double[][] xwLabel = new double[trainData.size()][trainData.get(0).size()];
			double[] diff = new double[y.size()];
			//double[] num = new double[trainData.get(0).size()];
			double num;
			double den;
		...
		/*
		public static double accuracyCalc(Double[] w, List<Integer> y, List<ArrayList<Double>> inData){
			
			double correctPred = 0;
			double accuracy = 0;
			double[] pred = new double[y.size()];
			List<Double> temp = new ArrayList<Double>();
			int[] label = new int[y.size()];
			
			for(int j = 0 ; j < inData.size();j++){
				temp = inData.get(j);
				for(int k = 0 ; k< inData.get(0).size(); k++){				
						pred[j] = pred[j] + temp.get(k)*w[k];				
				}
			}
		...
		*/
		...
		public static void main(String[] args) throws IOException  {
			...
			System.out.println(selectedLambda);
